package com.paligot.kighlighter.palettes

import com.paligot.kighlighter.core.Color
import com.paligot.kighlighter.core.rgb

object DarculaColorPalette: Palette {
    val black: Color = rgb(43, 43, 43)
    val blackoff: Color = rgb(50, 50, 50)
    val grey: Color = rgb(128, 128, 128)
    val greyLight: Color = rgb(169, 183, 198)
    val blueLight: Color = rgb(104, 151, 187)
    val blue: Color = rgb(70, 124, 218)
    val green: Color = rgb(106, 135, 89)
    val khaki: Color = rgb(187, 181, 41)
    val yellow: Color = rgb(255, 198, 96)
    val orange: Color = rgb(204, 120, 50)
}
